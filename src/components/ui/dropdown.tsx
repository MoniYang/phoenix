...
const Dropdown: React.FC<DropdownProps> = ({
	...
	const [selectedCurrent, setSelectedCurrent] = useState<Option>(
	...
	return (
		...
		>
			<button
				className={dropdownButton({ variant, open: isOpen })}
				onClick={toggleDropdown}
			>
				{selectedCurrent.img && (
					<img
						src={selectedCurrent.img}
						alt={selectedCurrent.text}
						className="h-5 mx-auto pl-1 border-l"
					/>
				)}
				{selectedCurrent.text && (
					<span className="px-1">{selectedCurrent.text}</span>
				)}
